...
select bidder_id,
sum(case when bid_status='won' then 1
else 0
end )/count(bidder_id)*100 win_percentage
...
select  ist.*, count(ims.match_id) No_of_matches
from ipl_match_schedule ims left join ipl_stadium ist
...
end)/count(match_id) as 'percentage of wins by a team which has won the toss'
from ipl_match
group by team_id1;
...
select ibp.bidder_id, no_of_bids as total_bids, ibd.bid_team, it.*
from ipl_bidder_points ibp left join ipl_bidding_details ibd
on ibp.bidder_id = ibd.bidder_id
left join ipl_match_schedule ims 
on ibd.schedule_id = ims.schedule_id
left join ipl_match im
...
select im.*, it.*
from ipl_match im left join ipl_team it
on im.match_winner = it.team_id;

# 6.	Display total matches played, total matches won and total matches lost by team along with its team name.
select * from ipl_team_standings;

select its.team_id, it.team_name, sum(matches_played) total_matches_played, sum(matches_won) total_matches_won, sum(matches_lost) total_matches_lost
from ipl_team_standings its left join ipl_team it
on its.team_id = it.team_id
group by its.team_id,team_name;

# 7.	Display the bowlers for Mumbai Indians team.

select itp.* , ip.player_name 
from ipl_team_players itp left join ipl_player ip
on itp.player_id = ip.player_id
where itp.remarks = 'team - mi' and itp.player_role = 'bowler';


select itp.* , ip.player_name , it.team_name
from ipl_team_players itp left join ipl_player ip
on itp.player_id = ip.player_id
left join ipl_team it
...
# n8.	How many all-rounders are there in each team, Display the teams with more than 4 
# all-rounder in descending order.

select team_id,player_role, count(player_id) from ipl_team_players 
where player_role = 'all-rounder' group by team_id having count(player_id) >4 
